    ...
    def _populate_devices(self) -> None:
        ...
        try:
            ...
            for idx, device_name in input_devices:
                self.device_combo.addItem(f"{device_name}", userData=idx)
            logger.info(
                "SettingsDialog detected {} input devices (combo size={})",
                len(input_devices),
                self.device_combo.count(),
            )
        except Exception as exc:
            logger.warning("SettingsDialog failed to enumerate audio devices: {}", exc)
